...
output_path_0 = hdfs_path + "output/step0/"
output_path_1 = hdfs_path + "output/step1/"

if __name__ == "__main__":
    sc = SparkContext(appName="discount analysis")
    spark = SparkSession(sc)
    input_df = spark.read.csv(hdfs_path + "input.txt")
    # input_list = input_df.rdd.map(lambda x: x[0]).collect()
    # input_df_pd = input_df.toPandas()
    # input_list = input_df_pd[input_df_pd.columns[0]].to_list()
    input_list = [row[input_df.columns[0]]
                  for row in input_df.select(input_df.columns[0]).collect()]

    #! Test for 3 files since already had output at local
